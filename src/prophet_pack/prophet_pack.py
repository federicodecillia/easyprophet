...
def create_grid(param_grid: dict) -> object:
    '''generate a grid, containing all the combination of parameters passed in the param_grid parameter.
...
def create_ts(df: pd.DataFrame, date_var: str, y_var: str, x_vars: list = False, sort_ts: bool = False):
    ''' transform the input df in order to be used by prophet, i.e. keeping just "ds" (datestamp),
        "y_var" and optional "x_vars" columns.
    ...
    Args:
        ...
        sort_ts: Boolean. False if df is already sorted (ascending). Otherwise true to sort it (+ computation time)

    Returns:
         df containing the ts ready to be fit by prophet
    '''
...
def detect_anomalies(forecast: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns:
        df containing predictions + "anomaly" & "importance" variables
    '''
    df = forecast.copy()
    df['anomaly'] = 0
...
def diagnostic_metrics(forecast: pd.DataFrame, actual: pd.DataFrame):
    '''Calculate diagnostics metrics (ME, MAE, MAPE, MPE, MSE, RMSE) comparing forecast values and actual values.
...
def list_seasonalities(yearly_seasonality: str = False,
    ...
    Args:
        ...
        holidays_country: name of the country of which load standard holidays (e.g. "US")
        x_vars: List of exogenous regressors used to fit y. Note: To predict the future,
            a dataframe of exogenous regressors is required
...
def prophet_pipeline(df: pd.DataFrame,
                     var: str,
                     rim_cat: str,
                     df_holidays: pd.DataFrame = None,
                     freq_data: str = "D",
